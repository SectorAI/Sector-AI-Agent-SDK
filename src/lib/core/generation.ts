...
export async function generateText({
  ...
  const temperature =
    modelConfiguration?.temperature || models[provider].settings.temperature;
    ...
    switch (provider) {
      ...
      case ModelProviderName.AKASH_CHAT_API: {
        ...
        const { text: openaiResponse } = await aiGenerateText({
          ...
          prompt: context,
          system:
            runtime.character.worldInfo ?? settings.SYSTEM_PROMPT ?? undefined,
          tools: tools,
      ...
      case ModelProviderName.ETERNALAI: {
        ...
        const openai = createOpenAI({
          ...
          fetch: async (url, options) => {
            ...
            if (
              parseBooleanFromText(runtime.getSetting("ETERNAL_AI_LOG_REQUEST"))
            ) {
              elizaLogger.info(
                "Request data: ",
