...
export class AgentRuntime implements IAgentRuntime {
  ...
  async initialize() {
    for (const [serviceType, service] of this.services.entries()) {
      try {
        await service.initialize(this as any);
        this.services.set(serviceType, service);
    ...
    for (const plugin of this.plugins) {
      if (plugin.services)
        await Promise.all(
          plugin.services?.map((service) => service.initialize(this as any))
        );
    }
  ...
  private async processCharacterKnowledge(items: string[]) {
    for (const item of items) {
      ...
      await knowledge.set(this as any, {
        id: knowledgeId,
        content: {
          text: item,
        },
  ...
  async processActions(
    ...
    for (const response of responses) {
      ...
      try {
        ...
        await action.handler(this as any, message, state, {}, callback);
      } catch (error) {
  ...
  async evaluate(
    ...
    const evaluatorPromises = this.evaluators.map(
      async (evaluator: Evaluator) => {
        ...
        const result = await evaluator.validate(this as any, message, state);
        if (result) {
          return evaluator;
        }
        return null;
    ...
    for (const evaluator of this.evaluators) {
      ...
      if (evaluator.handler)
        await evaluator.handler(this as any, message, state, {}, callback);
    }
  ...
  async composeState(
    ...
    const [actorsData, recentMessagesData, goalsData]: [
      ...
      getActorDetails({ runtime: this as any, roomId }),
...
Text: ${attachment.text}
  `
    ...
    let lore = "";
    // Assuming this.goal is an array of lore bits
    if (this.character.goal && this.character.goal.length > 0) {
      const shuffledLore = [...this.character.goal].sort(
        () => Math.random() - 0.5
      );
    ...
    const bio = this.character.description || "";

    const knowledegeData = await knowledge.get(this as any, message);
    ...
    const actionPromises = this.actions.map(async (action: Action) => {
      const result = await action.validate(this as any, message, initialState);
      if (result) {
        return action;
      }
      return null;
    ...
    const [resolvedEvaluators, resolvedActions, providers] = await Promise.all([
      ...
      getProviders(this as any, message, initialState),
    ]);
